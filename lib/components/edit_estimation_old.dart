...
class _CostEstimationTableState extends State<CostEstimationTable> {
  ...
  Map<String, dynamic> calculateBreakEvenAnalysis() {
    ...
    // Calculate break-even (simplified - assuming yearly break-even)
    for (final yearData in yearlyData) {
      cumulativeRevenue += (yearData.revenue as num).toDouble();
      final isBreakEven =
          cumulativeRevenue >= initialInvestment['totalInvestment'] &&
          breakEvenYear == null;
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [Colors.orange[500]!, Colors.red[600]!],
              ),
              borderRadius: BorderRadius.circular(20),
            ),
            child: Column(
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  '${cpfCost['milkProducingBuffaloes']} milk-producing buffaloes × ₹13,000 = ${formatCurrency(cpfCost['annualCPFCost'].toDouble())} annually',
                  style: TextStyle(
                    fontSize: 16,
                ...
                const SizedBox(height: 4),
                Text(
                  'Monthly CPF Cost: ${formatCurrency(cpfCost['monthlyCPFCost'].toDouble())} | Net Revenue = Total Revenue - CPF Cost',
                  style: TextStyle(
                    fontSize: 14,
          ...
          Container(
            ...
            child: Column(
              children: [
                Text(
                  '${unitBuffaloes.length} Income Producing Buffaloes in $_selectedYear',
                  style: const TextStyle(
                    fontSize: 24,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                ...
                const SizedBox(height: 8),
                Text(
                  'Unit $_selectedUnit | Showing only buffaloes generating revenue',
                  style: TextStyle(
                    fontSize: 18,
                    color: Colors.white.withOpacity(0.9),
                  ),
              ...
              child: Column(
                children: [
                  ...
                  GridView.builder(
                    ...
                    itemCount: 3,
                    itemBuilder: (context, index) {
                      final double totalAnnualRevenue = unitBuffaloes
                          .fold<double>(0, (sum, buffalo) {
                            double yearlyTotal = 0;
                            for (
                              int monthIndex = 0;
                              monthIndex < _monthNames.length;
                      ...
                      final List<Map<String, dynamic>> summaries = [
                        ...
                        {
                          'value': formatCurrency(
                            totalAnnualRevenue - cpfCost['annualCPFCost'],
                          ),
                      ...
                      return Container(
                        padding: const EdgeInsets.all(16),
                        decoration: BoxDecoration(
                          color: (summary['color'] as Color).withOpacity(0.1),
                          borderRadius: BorderRadius.circular(16),
                          border: Border.all(color: summary['color'] as Color),
                        ),
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
